...
def get_logo_uri() -> str:
    ...
    Warning is emitted by the caller (inject_branding) so it fires every
    run, not just the first (cached) invocation.
    """
...
def inject_branding() -> None:
    ...
    ``st.set_page_config()``.  Not idempotent — call exactly once per
    ``run()`` invocation.  Multiple calls will inject duplicate CSS blocks
    (Streamlit does not deduplicate identical ``st.markdown`` injections).

    Also emits asset-missing warnings if logo or icon SVGs cannot be
    resolved from disk (warnings suppressed inside cached getters).
    """
