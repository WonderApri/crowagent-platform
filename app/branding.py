...
/* ── Segment Icon Alignment (all profiles) ─────────────────────────────── */
.segment-title-row {
  display: flex;
  align-items: center;
  gap: 10px;
...
.segment-icon-badge {
  ...
  font-size: 1.1rem;
  line-height: 1;
  background: linear-gradient(160deg, #FF7A78 0%, #F04F59 100%);
  color: #ffffff;
  box-shadow: 0 2px 8px rgba(240, 79, 89, 0.28);
  flex-shrink: 0;
}
.segment-title {
  margin: 0;
...
/* ── Weather Card — Soft overcast look ─────────────────────────────────── */
.weather-card-overcast {
  background: linear-gradient(180deg, #EBF2F7 0%, #D8E3ED 100%);
